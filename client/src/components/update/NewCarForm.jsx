...
function NewCarForm({ closeModal, refetchCars }) {
  ...
  const uploadFiles = async () => {
    ...
    files.forEach(file => {
      console.log('Uploading file inside the for each:', file);
      formData.append('file', file);
    });
    try {
      const response = await axios.post('http://localhost:3001/s3upload', formData);
      //https://usa.alpineauto.xyz/upload
      console.log('File upload response:', response);

      if (response.status === 200) {
        // Aquí asumiendo que la URL o identificador del archivo debe obtenerse de "results"
        const uploadedImagePaths = response.data.results.map(key => {
          // Dependiendo de lo que devuelve S3, puede que quieras extraer una URL aquí
          const region = import.meta.env.VITE_AWS_REGION;
          const bucketName = import.meta.env.VITE_AWS_BUCKET_NAME; // Ensure you have this environment variable set
        ...
        console.error('No success status in response:', response.data);
        return [];
      }
    } catch (error) {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="w-full pr-2 md:px-3 mt-3 overflow-hidden overflow-y-scroll z-10">
      ...
      <div className="flex justify-center">
        <Button type="submit" className="bg-red-500 text-white hover:bg-red-700 transition-colors hover:font-bold">Submit</Button>
      </div>
